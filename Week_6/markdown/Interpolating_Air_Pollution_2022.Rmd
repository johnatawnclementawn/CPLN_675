...
epa <- st_read("D:/Users/Johnathan/Google Drive/Grad School/Penn_MUSA/Spring2022/675_LUaEModeling/Github/Week_6/675_wk6_inclassEx/Mid_Atlantic_EPA_Dataset_withLag.shp") %>%
...
- From the original `epa` layer we pull out just the variables we want for this analysis using `select`. Examine what we keep - some variables associated with the landscape and land use, and our Ozone lag `MEAN_ozone`.
...
reg <- lm(ozoneHigh ~ ., data=training %>% 
                                   as.data.frame() %>%
                                   select(-MEAN_ozone,-geometry,-training,-X,-Y))
summary(reg)
...
reg2 <- lm(ozoneHigh ~ ., data=training %>% 
                                   as.data.frame() %>%
                                   select(-MEAN_ozone,-geometry,-training))
summary(reg2)
```
...
```{r}
ggplot(training, aes(MEAN_ozone, ozoneHigh)) +
 geom_point() +
...
ggplot(data = training.summary, aes(ozoneHigh, Value)) + 
...
grid.arrange(ggplot() +
    geom_sf(data = epa_joined, aes(fill=MEAN_ozone), color = "transparent") + 
